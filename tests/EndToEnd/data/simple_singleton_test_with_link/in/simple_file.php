...
function doThingOne() {
    $result1 = mysql_query('insert into visit (time) VALUES (now())', BdConnection::getConnection());
    if(mysql_affected_rows(BdConnection::getConnection()) == 1) {
        $visit = mysql_insert_id(BdConnection::getConnection());
    }
}
...
function doThingTwo() {
    ...
    . mysql_real_escape_string('a"string"', $link) . ' ORDER by id',  BdConnection::getConnection());
